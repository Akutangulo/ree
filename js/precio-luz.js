...
fetch(apiUrl)
    ...
    .then(data => {
        ...
        if (!pricesData.some(item => new Date(item.datetime).getDate() === tomorrowDate.getDate())) {
            // Si no hay datos para mañana, ocultar el contenedor de mañana
            containerTomorrow.style.display = 'none';
        }
        ...
        infoToday.innerHTML = `<span class="precioMedioLuz">El precio medio de la luz hoy ${formattedCurrentDate} es de ${avgPriceToday} €</span><br>` +
            ...
            `<br><span class="precioLuzCaro">Precio más caro hoy: ${priceValuesToday[23]} € a las ${getCurrentTime(new Date(pricesData.find(item => item.value === priceValuesToday[23]).datetime))}</span>`;
        ...
        infoTomorrow.innerHTML = `El precio medio de la luz mañana ${formattedTomorrowDate} es ${avgPriceTomorrow} €<br>` +
            ...
            `<span class="precioLuzCaro">Precio más caro mañana: ${priceValuesTomorrow[23]} € a las ${getCurrentTime(new Date(pricesData.find(item => item.value === priceValuesTomorrow[23]).datetime))}</span>`;
    })
...
// Función para manejar el cambio en el estado del boton-checkbox
function togglePrecioPasado() {
    var checkbox = document.getElementById("precioLuzPasado");
    var precioPasadoDivs = document.getElementsByClassName("precioPasado");

    for (var i = 0; i < precioPasadoDivs.length; i++) {
        if (checkbox.checked) {
            precioPasadoDivs[i].style.display = "none";
        } else {
            precioPasadoDivs[i].style.display = "block"; 
        }
    }
}
